...
class GdxSceneFileService : SceneFileService {
    ...
    override fun readText(path: String): String = resolveReadableFile(path).readString("UTF-8")
    ...
    override fun exists(path: String): Boolean = Gdx.files.local(path).exists() || Gdx.files.internal(path).exists()
