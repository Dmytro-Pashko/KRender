...
import com.pashkd.krender.engine.tools.uicomposer.UiComposerGuideRect
import com.pashkd.krender.engine.tools.uicomposer.UiComposerGuideSnapshot
import com.pashkd.krender.engine.tools.uicomposer.UiComposerVisualGuideOptions
...
class GdxUiScenePreview(
    ...
    private fun renderVisualGuides() {
        ...
        val selectedAlign = selectedNode?.align
        if (
            visualGuideOptions.showAlignmentGuide &&
            selectedActor != null &&
            selectedAlign != null
        ) {
            val anchor = computeAlignmentAnchor(actorStageRect(selectedActor), selectedAlign)
            drawText("align: $selectedAlign", anchor.x + 6f, anchor.y + 14f, AlignmentGuideColor)
        }
        guideBatch.end()
    }
