...
class BitmapFontParser {
    ...
    private fun buildDocument(fntFile: File, lines: List<String>): BitmapFontDocument {
        ...
        if (common == null) {
            diagnostics += diag(TextureAtlasEditorDiagnosticSeverity.Error, "Missing 'common' block in font descriptor.", fntPath)
        }
        if (pages.isEmpty()) {
            diagnostics += diag(TextureAtlasEditorDiagnosticSeverity.Error, "No font pages declared.", fntPath)
        }
        if (glyphs.isEmpty()) {
            diagnostics += diag(TextureAtlasEditorDiagnosticSeverity.Warning, "Font has no glyphs.", fntPath)
        }
        pages.forEach { page ->
            if (!page.exists) {
                diagnostics += diag(TextureAtlasEditorDiagnosticSeverity.Warning, "Page texture '${page.file}' not found.", fntPath)
            }
        }
        ...
        val hasErrors = diagnostics.any { it.severity == TextureAtlasEditorDiagnosticSeverity.Error }
    ...
    private fun parseCommon(
        ...
        if (lineHeight == null || base == null || scaleW == null || scaleH == null || pages == null) {
            diagnostics += diag(TextureAtlasEditorDiagnosticSeverity.Error, "Incomplete 'common' block.", source)
            return null
        }
    ...
    private fun parsePage(
        ...
        if (id == null || fileName.isNullOrBlank()) {
            diagnostics += diag(TextureAtlasEditorDiagnosticSeverity.Warning, "Malformed page entry.", source)
            return null
        }
    ...
    private fun parseGlyph(
        ...
        val id = attrs["id"]?.toIntOrNull()
        if (id == null) {
            diagnostics += diag(TextureAtlasEditorDiagnosticSeverity.Warning, "Glyph missing id at line ${lineIndex + 1}.", source)
            return null
        }
    ...
    private fun parseKerning(
        ...
        if (first == null || second == null || amount == null) {
            diagnostics += diag(TextureAtlasEditorDiagnosticSeverity.Warning, "Malformed kerning entry.", source)
            return null
        }
    ...
    private fun validateGlyphBounds(
        ...
        glyphs.forEach { glyph ->
            if (glyph.page !in pageIds && pageIds.isNotEmpty()) {
                diagnostics += diag(TextureAtlasEditorDiagnosticSeverity.Warning, "Glyph id=${glyph.id} references missing page ${glyph.page}.", source)
            }
            if (glyph.width > 0 && glyph.height > 0) {
                ...
                if (glyph.x < 0 || glyph.y < 0 || right > common.scaleW || bottom > common.scaleH) {
                    diagnostics += diag(TextureAtlasEditorDiagnosticSeverity.Warning, "Glyph id=${glyph.id} is outside page bounds.", source)
                }
            }
        }
    }
