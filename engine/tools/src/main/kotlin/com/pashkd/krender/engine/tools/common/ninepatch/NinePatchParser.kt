...
class NinePatchParser {
    ...
    private fun parseHorizontalBorder(
        ...
        for (x in 1 until image.width - 1) {
            when (classifyArgbPixel(image.pixelAt(x, y))) {
                GuidePixel.Guide -> if (segmentStart == null) segmentStart = x - 1
                GuidePixel.Invalid -> {
                    invalidPositions += (x - 1)
                    segmentStart = closeHorizontalSegment(segments, segmentStart, x)
                }
                GuidePixel.Transparent -> segmentStart = closeHorizontalSegment(segments, segmentStart, x)
            }
        }
        ...
        if (invalidPositions.isNotEmpty()) {
            issues += issue(NinePatchValidationSeverity.Warning, "The $label guide contains non-black opaque pixels at drawable positions ${invalidPositions.joinToString(", ")}.")
        }
    ...
    private fun parseVerticalBorder(
        ...
        for (y in 1 until image.height - 1) {
            when (classifyArgbPixel(image.pixelAt(x, y))) {
                GuidePixel.Guide -> if (segmentStart == null) segmentStart = y - 1
                GuidePixel.Invalid -> {
                    invalidPositions += (y - 1)
                    segmentStart = closeVerticalSegment(segments, segmentStart, y)
                }
                GuidePixel.Transparent -> segmentStart = closeVerticalSegment(segments, segmentStart, y)
            }
        }
        segmentStart?.let { start ->
    ...
    private fun selectPaddingSegment(
        ...
        if (segments.size > 1) {
            issues += issue(NinePatchValidationSeverity.Warning, "Multiple content/padding guide segments were found on the $borderLabel border; using the first segment only.")
        }
    ...
    private fun validateBounds(
        ...
        segments
            .filter { segment -> segment.length <= 0 || segment.start < 0 || segment.endInclusive >= size }
            .forEach { segment ->
                issues += issue(NinePatchValidationSeverity.Warning, "$label segment ${segment.start}..${segment.endInclusive} is outside drawable bounds.")
            }
    }
    ...
    private fun classifyArgbPixel(pixel: Int): GuidePixel {
        ...
        return if (red <= BlackThreshold && green <= BlackThreshold && blue <= BlackThreshold) GuidePixel.Guide else GuidePixel.Invalid
    }
