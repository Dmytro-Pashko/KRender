...
val mainClassName = "com.pashkd.krender.lwjgl3.MacOsLwjgl3Launcher"
...
application {
    ...
    applicationName = "$appName-macos"
}

eclipse.project.name = "$appName-desktop-lwjgl3-macos"
...
tasks.named<Jar>("jar") {
    archiveFileName.set("$appName-$krenderVersion-macos.jar")
...
tasks.register("jarMac") {
    group = "build"
    description = "Builds the macOS desktop launcher JAR."
    dependsOn(tasks.named("jar"))
}

tasks.register("dist") {
    group = "build"
    description = "Builds the macOS desktop distribution JAR using the legacy gdx-setup task name."
    dependsOn(tasks.named("jar"))
}
