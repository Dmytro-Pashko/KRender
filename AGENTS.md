...
`scenes`, `assets`, `sceneFiles`, `runtimeLauncher`, `editorToolLauncher`, `input`,
...
a `SceneState`. Lifecycle hooks: `load` (suspend, **currently never called — see warning
in §8**), `scheduleAssets`, `show`, `fixedUpdate`, `update`, `lateUpdate`, `render`,
`debugRender`, `overlayRender`, `resize`, `hide`, `dispose`.
...
> **Verified caveat — important for agents:** Although `Scene.load(context)` is documented
> as "Stage A: CPU/IO-only preparation", **`SceneManager.activateScene` never calls it**, and
> no scene currently overrides it. `SceneState.Preparing` is likewise never assigned.
> Effective lifecycle today is `scheduleAssets` → `show`. Assets load **asynchronously** while
> the scene is already `Active` (the loop calls `assets.update()` each frame), so scenes must
> tolerate "not loaded yet" — viewer tools poll `AssetService.isLoaded` and show a Loading panel.
...
- **Editor asset discovery** is a different system: `LocalAssetRegistryService`
  ...
  thread. This feeds the Asset Browser and Scene Editor asset panel.
...
### Animation Viewer
`game/AnimationViewerScene.kt` (+ `engine/animationviewer/`). Plays model animation clips and
...
| Backend bootstrap | `GdxEngineApplication` | `backend/gdx/LibGdxBackend.kt` | libGDX `ApplicationAdapter` that boots runtime. |
...
| Render impl | `GdxRenderer3D` | `backend/gdx/LibGdxBackend.kt` | Consumes commands via `ModelBatch`. |
| Assets | `AssetService`, `AssetRef`, `AssetPack` | `api/Assets.kt` | Typed asset handles + loading. |
| Assets impl | `GdxAssetService` | `backend/gdx/LibGdxBackend.kt` | libGDX `AssetManager` loading + metadata. |
...
| Launchers | `EditorToolLauncher`, `RuntimeWindowLauncher` | `scene/` + `lwjgl3/` | Open tool/runtime windows. |
...
- If you wire up `Scene.load()` or `Preparing` (currently dormant), update `SceneManager`,
  this guide, and add tests — do not silently change lifecycle semantics.
