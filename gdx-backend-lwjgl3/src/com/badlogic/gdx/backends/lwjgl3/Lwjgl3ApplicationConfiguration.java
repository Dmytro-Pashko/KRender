...
public class Lwjgl3ApplicationConfiguration extends Lwjgl3WindowConfiguration {
  ...
  /**
   * Sets which OpenGL version to use to emulate OpenGL ES. If the given major/minor version is
   * not supported, the backend falls
   * back to OpenGL ES 2.0 emulation through OpenGL 2.0. The default parameters for major and
   * minor should be 3 and 2
   * respectively to be compatible with Mac OS X. Specifying major version 4 and minor version 2
   * will ensure that all OpenGL ES
   ...
   */
  public void setOpenGLEmulation(GLEmulation glVersion, int gles3MajorVersion,
   ...
   * Use 0 to never sleep. Default is
   * 0.
   */
   ...
   * used to store them. Defaults to
   * "$USER_HOME/.prefs/" and {@link FileType#External}.
   */
  public void setPreferencesConfig(String preferencesDirectory,
   ...
   * may report window width/height and mouse coordinates in a logical coordinate system at a
   * lower resolution than the actual
   * physical resolution. This setting allows you to specify whether you want to work in logical
   * or raw pixel units. See
   * {@link HdpiMode} for more information. Note that some OpenGL functions like
   * {@link GL20#glViewport(int, int, int, int)} and
   * {@link GL20#glScissor(int, int, int, int)} require raw pixel units. Use {@link HdpiUtils} to
   * help with the conversion if
  ...
  /**
   * Enables use of OpenGL debug message callbacks. If not supported by the core GL driver (since
   * GL 4.3), this uses the
   * KHR_debug, ARB_debug_output or AMD_debug_output extension if available. By default, debug
   ...
   * Use
   *
	 * {@link Lwjgl3Application#setGLDebugMessageControl(Lwjgl3Application.GLDebugMessageSeverity, boolean)} to enable or
  ...
  public static DisplayMode getDisplayMode() {
    ...
    return new Lwjgl3Graphics.Lwjgl3DisplayMode(GLFW.glfwGetPrimaryMonitor(), videoMode.width(),
        videoMode.height(),
  ...
  public static DisplayMode getDisplayMode(Monitor monitor) {
    ...
    return new Lwjgl3Graphics.Lwjgl3DisplayMode(((Lwjgl3Monitor) monitor).monitorHandle,
  ...
  public static DisplayMode[] getDisplayModes() {
    ...
    for (int i = 0; i < result.length; i++) {
      ...
      result[i] = new Lwjgl3Graphics.Lwjgl3DisplayMode(GLFW.glfwGetPrimaryMonitor(),
          videoMode.width(), videoMode.height(),
          videoMode.refreshRate(),
          videoMode.redBits() + videoMode.greenBits() + videoMode.blueBits());
    }
  ...
  public static DisplayMode[] getDisplayModes(Monitor monitor) {
    ...
    for (int i = 0; i < result.length; i++) {
      GLFWVidMode videoMode = videoModes.get(i);
      result[i] = new Lwjgl3Graphics.Lwjgl3DisplayMode(((Lwjgl3Monitor) monitor).monitorHandle,
          videoMode.width(),
