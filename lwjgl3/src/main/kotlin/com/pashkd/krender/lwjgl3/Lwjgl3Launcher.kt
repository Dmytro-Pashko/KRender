...
private fun Lwjgl3ApplicationConfiguration.configureOpenGlTarget() {
    val target = System.getProperty("krender.gl.target")
...
private fun desktopBoolean(name: String, default: Boolean): Boolean {
...
private fun desktopInt(name: String): Int? =
    desktopProperty(name)?.trim()?.toIntOrNull()

private fun desktopProperty(name: String): String? =
    System.getProperty(name) ?: System.getenv(name.replace('.', '_').uppercase(Locale.ROOT))
