...
class SceneEditorScene(
    ...
    private fun createUiSystem(
        ...
        UiSystem(engine.ui).also { uiSystem ->
            uiSystem.addPanel(SceneEditorToolbarPanel(editorState, layoutConfig, panelEventLogger))
            uiSystem.addPanel(SceneHierarchyPanel(editorState, world, layoutConfig, panelEventLogger))
            uiSystem.addPanel(SceneInspectorPanel(editorState, world, layoutConfig, panelEventLogger))
            uiSystem.addPanel(SceneViewportPanel(editorState, world, layoutConfig, panelEventLogger))
            uiSystem.addPanel(LogsPanel(engine.logs, layoutConfig, panelEventLogger))
        }

    private fun createDefaultCamera() {
        ...
        editorState.selectedEntityId = camera.id
    }

    private fun createDefaultLights() {
