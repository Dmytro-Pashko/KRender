...
class PlayerControllerSystem(
    ...
    override fun update(world: SceneWorld, dt: Float) {
        ...
        if (moving) {
            // Normalize keyboard input first so diagonal movement is not faster.
            val inputLength = sqrt(inputX * inputX + inputZ * inputZ)
            inputX /= inputLength
            inputZ /= inputLength

            // Project the active camera's look direction onto X/Z and build movement from that basis.
        ...
        if (!snapshot.uiCapturesKeyboard && snapshot.wasPressed(Key.Space) && controller.isGrounded) {
            // Jump is a one-shot state transition; height is computed by updateJump().
    ...
    private fun updateJump(
        ...
        // Simple symmetric parabola: starts and ends at PlayerBaseY, peaks at JumpHeight.
    ...
    /**
     * Builds a horizontal camera-relative movement basis from the active camera.
     *
     * Prefer [PerspectiveCameraComponent.lookAt] because the orbit camera keeps
     * it synced to the player. Euler yaw is only a fallback for authored cameras
     * or tests that do not set a look-at target.
     */
    ...
    /** Returns a normalized horizontal forward vector for authored cameras without lookAt. */
...
class CharacterAnimationSystem(
    ...
    private fun horizontalSpeedSquared(velocity: VelocityComponent): Float =
